...
function Users() {
  ...
  useEffect(() => {
    if (userRole === "admin") {
      fetch("https://back001pwa.onrender.com/auth/users")
        .then((response) => {
  ...
  const registerServiceWorker = async () => {
    try {
      ...
      const response = await fetch("https://back001pwa.onrender.com/auth/suscripcion", {
        method: "POST",
  ...
  const handleSendMessage = async (user) => {
    try {
      ...
      const response = await fetch("https://back001pwa.onrender.com/auth/suscripcionMod", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          suscripcion: user.suscripcion,
          nombre: user.nombre,
          mensaje: `Gracias ${user.nombre} Mensaje: ${message}`,
        }),
      });
  ...
  return (
    <div className="page-container">
      ...
      {userRole === "admin" ? (
        <div>
          <h2 style={{textAlign: 'center', color: 'white'}}>📋 Registros fut...</h2>
          {isLoading ? (
        ...
        <p style={{ color: 'white', fontSize: '20px' }}>Zona Futbolera {userNombre} Bienvenido!! 😊</p>
      )}
    </div>
  );
}
